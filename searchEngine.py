...
def compute_ranks(graph):
    d = 0.8 # damping factor 
            # - prob that a user selects a link from current page,
            # rather than jumping to a completely new page
    numloops = 1 # number of times to go through the relaxation
    
    ranks = {}
    npages = len(graph)
    
    # initialise ranks (uniform probabilities)
    for page in graph:
...
def lookup(key, index, graph, ranks):
    if not key in index:
        return None
    urls = index[key]
    urlrank = {}
    for url in urls:
        urlrank[url] = ranks[url]
    result = sorted(urlrank.iteritems(), key=operator.itemgetter(1))
    result.reverse()
    return result
...
# EXAMPLE - search for...
key = 'Hummus'
results = lookup(key, index, graph, ranks)
